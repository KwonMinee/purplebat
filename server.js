...
// 게시글 본문에 이미지를 base64로 직접 넣을 수 있어서 기본 100kb 제한을 넉넉하게 늘렸습니다.
...
// ---- admin.html / admin-write.html은 로그인한 관리자만 접근 가능 (정적 서빙보다 먼저 가로챔) ----
function requireAdminPage(req, res, next) {
  if (req.session && req.session.isAdmin) return next();
  return res.redirect("/admin-login.html");
}
app.get("/admin.html", requireAdminPage);
app.get("/admin-write.html", requireAdminPage);
